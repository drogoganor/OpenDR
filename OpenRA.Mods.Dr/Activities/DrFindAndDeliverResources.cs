...
{
	public class DrFindAndDeliverResources : Activity
	{
		...
		public DrFindAndDeliverResources(Actor self, CPos orderLocation)
			: this(self, null)
		{
		...
		public override bool Tick(Actor self)
		{
			...
			if (LastSearchFailed)
			{
				...
				{
					var deliveryLoc = lastproc.Location + lastproc.Trait<IAcceptDrResources>().DeliveryOffset;
		...
		CPos? GetSearchFromProcLocation(Actor self)
		{
			if (harv.LastLinkedProc != null && !harv.LastLinkedProc.IsDead && harv.LastLinkedProc.IsInWorld)
				return harv.LastLinkedProc.Location + harv.LastLinkedProc.Trait<IAcceptDrResources>().DeliveryOffset;

			if (harv.LinkedProc != null && !harv.LinkedProc.IsDead && harv.LinkedProc.IsInWorld)
				return harv.LinkedProc.Location + harv.LinkedProc.Trait<IAcceptDrResources>().DeliveryOffset;
